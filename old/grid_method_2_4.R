...
end_points <- function(circle_x, circle_y, circle_radius, radius, chord) {
  ...
  # y^2 = r - x^2
  # y = +/- sqrt(r - x^2)
  y1 = sqrt(circle_radius^2 - x1^2)
  y2 = -sqrt(circle_radius^2 - x2^2)
  
  # now rotate these points around to the radius
  x1_moved = cos(radius) * x1 - sin(radius) * y1
  y1_moved = sin(radius) * x1 + cos(radius) * y1
  x2_moved = cos(radius) * x2 - sin(radius) * y2
  y2_moved = sin(radius) * x2 + cos(radius) * y2
  
  # move back to where the circle is
  x1_moved = x1_moved + circle_x
  y1_moved = y1_moved + circle_y
  x2_moved = x2_moved + circle_x
  y2_moved = y2_moved + circle_y
  
  return(data.frame(point_1_x = x1_moved,
                    point_1_y = y1_moved,
                    point_2_x = x2_moved, 
                    point_2_y = y2_moved))
}
...
pdf(file = file.path(here::here(), "method2_2.pdf"),
    width = 18,
    height = 24,
