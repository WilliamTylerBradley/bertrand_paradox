...
end_points <- function(circle_x, circle_y, circle_radius, radius, chord) {
  ...
  # forget all this
  # find the y values for when x = chord
  # then rotate to match radius
  x1 = chord*circle_radius
  x2 = chord*circle_radius
  
  # x^2 + y^2 = r
  # y^2 = r - x^2
  # y = +/- sqrt(r - x^2)
  y1 = sqrt(circle_radius - x1^2)
  y2 = -sqrt(circle_radius - x2^2)
  
  # now rotate these points around to the radius
  x1_rotated = cos(radius) * x1 - sin(radius) * y1
  y1_rotated = sin(radius) * x1 + cos(radius) * y1
  x2_rotated = cos(radius) * x2 - sin(radius) * y2
  y2_rotated = sin(radius) * x2 + cos(radius) * y2
  ...
  # rotate center of circle to be x = 0 and 
  
  return(data.frame(midpoint_x = x1_rotated,
                    midpoint_y = y1_rotated,
                    new_point_x = x2_rotated, 
                    new_point_y = y2_rotated))
}
...
ggplot(data = opts, aes(x = x, y = y)) +
  geom_line(aes(col = opt, group = opt)) +
