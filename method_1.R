...
x_limits <- c(0, 18)
y_limits <- c(0, 24)
line_color <- as(hex2RGB("#C4722C"), "polarLUV") # Dark orange - yellow
point_color <- as(hex2RGB("#FECE69"), "polarLUV") # Bright yellow - orange
...
# Image
##-----
agg_png(file = file.path(here::here(), "method_1.png"),
        width = 18,
        height = 24,
        units = "in",
        res = 144)
...
opts <- opts %>% 
  mutate(col1 = ifelse(opt == sample(opt, 1), 
                       ...
                       'white')) %>%
  group_by(col1) %>%
  mutate(col2 = ifelse(opt == sample(opt, 1) | 
                         col1 == hex(line_color), 
                       hex(line_color), 
                       'white')) %>%
  ungroup()
