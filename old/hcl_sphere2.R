...
# l ranges from 
L = seq(-l_range, l_range) + 
  coords(color)[, 'L']

# now h_range need to change according level of l
# basically, chord length along x based on value of y
# (x−a)^2 + (y−b)^2 = r^2
# (x−a)^2 = r^2 - (y−b)^2
# (x - a) = sqrt(r^2 - (y−b)^2)
# x = sqrt(r^2 - (y−b)^2) + a
# x = a + sqrt(r^2 - (y−b)^2), a - sqrt(r^2 - (y−b)^2)
H_min = coords(color)[, 'H'] + sqrt(l_range^2 - (L - coords(color)[, 'L'])^2)
