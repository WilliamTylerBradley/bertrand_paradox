...
get_hcl_data <- function(H_point, C_point, L_point, width, n_color) {
 data.frame(theta = 2 * pi * seq(0, n_color - 1) / ((1 + sqrt(5)) / 2),
  ...
  mutate(L_cut = as.character(
    cut(L, breaks = c(-Inf, seq(L_point - 15, 
                                L_point + 15, 
                                length.out = 7), 
                      Inf),
        labels = c(L_point - 15, 
...
get_C_L_plane_by_H <- function(color_points, H_point) {
...
C_L_plane_by_H <- get_C_L_plane_by_H(hcl, H_point)
...
get_H_L_curve_by_C <- function(color_points) {
  map_dfr(as.numeric(as.character(unique(hcl$C_cut))), get_H_L_curve) %>%
...
ggsave(here::here("output", "color_compare", "hcl_h_l_curve.png"), 
       width = 5, height = 2)
# Note: graphs should be getting wider as C increases
...
# C L plane by H
C_L_plane_by_H <- get_C_L_plane_by_H(xyl, H_point)
graph_C_L_by_H(xyl)
...
compare <- rbind(hcl[, c('H', 'C', 'L', 'x', 'y', 
                 ...
                 xyl[, c('H', 'C', 'L', 'x', 'y', 
                         ...
                         'perpendicular_from_C_L_cut', 'parallel_along_C_L_cut',
                         'row_value', 'col_value', 'color_value')]) %>%
  mutate(setting = c(rep('HCL', n_color), rep('XYL', n_color)))
