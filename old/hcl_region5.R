...
# df_random <- data.frame(u = runif(n = n_random), # H
#                         v = rbeta(n = n_random, shape1 = 2, shape2 = 1), # L
#                         w = runif(n = n_random)) %>% # C
#   mutate(theta = u * 2 * pi, # H
#          phi = acos(2 * v - 1), # L
#          r = w^(1/3)) %>% # C
#   mutate(L = r * sin(phi) * cos(theta),
#          C = r * sin(phi) * sin(theta),
#          H = r * cos(phi))
